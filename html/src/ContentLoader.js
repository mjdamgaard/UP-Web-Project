...
// This module implements the ContentLoader (CL) class. A CL's purpose is to
// generate HTML elements and to define functions, i.e. callbacks, that are
// called immediately after the HTML element is created. We here refer to any
// live HTML element generated by a CL as a "content instance," which will
// often be abbreviated as simply a "CI."
//
// The way that these content loaders are implemented is that they each have a
// HTML template that defines the initial HTML of their CIs. And these templates
// can furthermore hold placeholders with so-called "content keys" referencing
// other CL. These placeholders have the form "<<MyContentLoader>>" where
// "MyContentLoader" is then the "content key" of some CL.
// Thus, the CLs can be defined from other CLs, which also means that several
...
// template of one CL consists only of the content key (wrapped in "<<>>") of
// another CL. In that case, the latter CL will load first, namely since the
// callbacks of any child CIs will be called before its parent's callbacks
// (except for certain "data callbacks"). And then the parent CL will
// immediately call its own calbacks, which can there for change (or "decorate")
// this child CL into something else. Decorator CLs thus work much like
// descendant classes of OOP.
//
// Now, to actaully understand how the ContentLoader class works, it is
// recommended to study the actual CL modules of this project (see
// src/content_loaders/), rather than to immediately try to read the source
// code below, as this will give a much better understanding of the central
// principles of how it all works.
//
// WARNING: The htmlTemplate of a ContentLoader must always only contain one
// outer element (which can have en arbitrary number of descendant elements
// inside it, however). For instance a HTML template of
// "<div>foo</div><div>bar</div>" is not allowed (as it can result in weird
// bugs). Also, any placeholder of the form '<<MyCL data(.property)[...]>>',
// namely one that uses that '[...]' syntax at the end, must always be inside
// another element in the HTML template (since such placeholders will be
// replaced by a list of CIs, which therefore will break the same rule if these
