...
export const PagesUnderTabs = ({tabDataArr}) => {
  const [state, setState] = useState({
    currInd: 0,
    isLoadedArr: [true]
  });
  const {currInd, isLoadedArr} = state;

  const tabIDs = tabDataArr.map(val => val[0]);
  const tabTitles = tabDataArr.map(val => val[1]);
  ...
  return (
    <div className="pages-under-tabs">
      <TabList
        tabTitles={tabTitles}
        activeTab={activeTab}
...
// export const TappedSubpages = ({tabDataArr, initTab}) => {
