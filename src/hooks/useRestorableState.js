...
const stateDataStore =
  JSON.parse(sessionStorage.getItem("_restorableStates") ?? "null") ?? {};

function getPrevStateData(id, delimiter) {
  var stateDataObj = stateDataStore;
  idParts = id.split(delimiter);
  let len = idParts.length;
  for (let i = 0; i < len; i++) {
    stateDataObj = stateDataObj[idParts[i]][1];
    if (!stateDataObj) {
      break;
    }
  }
  return stateDataObj || stateDataObj[0];
} 

function setStateData(id, state, refs, delimiter) {
  var stateDataObj = stateDataStore;
  idParts = id.split(delimiter);
  let len = idParts.length;
  for (let i = 0; i < len - 1; i++) {
    stateDataObj = stateDataObj[idParts[i]][1];
    if (!stateDataObj) {
      debugger;throw "updateState(): Parent state is not defined.";
    }
  }
  let parentStateDataObj = stateDataObj;
  stateDataObj = parentStateDataObj[idParts[len - 1]] ?? [[], {}];
  stateDataObj[0] = [state, refs];
  parentStateDataObj[idParts[len - 1]] = stateDataObj;
} 


export const useRestorableState = (
  id, initState, refs
) => {

  // Get whether there is a restoration of an earlier state going on.
  const prevState = stateDataStore[id];
