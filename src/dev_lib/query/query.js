...
export const query = new DevFunction(
  ...
  function(
    ...
    return _query(
      extendedRoute, isPost, postData, options,
      callerNode, execEnv, interpreter
    );
  }
);
...
export async function _fetchPrivate(
  extendedRoute, options,
  callerNode, execEnv, interpreter,
  ancestorModules = [], finalCallbacks = [],
) {
  // Add isPrivate: true to the options object before calling _fetch().
  let modifiedOptions = {};
  forEachValue(options, callerNode, execEnv, (val, key) => {
    modifiedOptions[key] = val;
  });
