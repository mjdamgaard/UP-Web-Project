...
import {scssParser} from "./parsing/SCSSParser.js";
import {
...
export class SCSSTranspiler {
  ...
  transpile(
    ...
    let [styleSheetNode, lexArr, strPosArr] = parseString(
      styleSheet, callerNode, callerEnv, scssParser
    );

    // Create a new environment for the style sheet. (We can just use the same
    // Environment class as the ScriptInterpreter for the SCSS environments.)
    ...
    if (type === "variable-declaration") {
      ...
      if (
        ...
        if (val !== undefined) {
          ...
          // valid (and implemented) SCSS value.
          let [{error}] = scssParser.parse(val.toString(), "value");
          if (error) throw new RuntimeError(
            `Style parameter "${val}" is either not a valid SCSS value or is ` +
            "not implemented yet",
            stmt, environment
          );
        }
      ...
      "Unrecognized SCSS statement type: " + type
    );
  ...
  transpileSimpleSelector(
    ...
    if (type === "class-selector") {
      ...
      // an existing SCSS variable containing a route, then get that route, and
...
export const scssTranspiler = new SCSSTranspiler();


export {scssTranspiler as default};
